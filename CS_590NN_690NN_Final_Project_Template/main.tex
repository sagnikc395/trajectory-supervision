...
We compare a stripped-context next-API-call baseline against a trajectory-context baseline under the same model, seed, stream ordering, QLoRA setup, and evaluation code. We do not claim to have solved catastrophic forgetting or to have shown that API traces work like human thought. The contribution is narrower: a first controlled measurement of whether retaining tool-use trajectory information changes continual tool-call learning.
...
Lightman et al.\ compare process and outcome supervision for mathematical reasoning and show that step-level supervision can be more effective than final-answer supervision \citep{lightman2023verify}. Our work is inspired by that distinction, but our data is different. API trajectories are not annotated reasoning steps; they are records of tool calls and tool observations. This makes the supervision weaker as a cognitive model but easier to collect in real systems.
...
In this single-seed API-Bank stream, keeping trajectory context improves final held-out next-API-call generation. The improvement is strongest for tool selection. The action-observation history likely gives the model useful structure for choosing which API to call next.
...
We tested whether trajectory context changes continual tool-use learning in a controlled Llama 3.1 8B QLoRA pilot. The full held-out evaluation shows a clear advantage for trajectory context on final exact API-call generation: 56.9\% versus 39.2\%. This remains a pilot because the trajectory condition uses more tokens and the experiment uses one seed.

The most urgent follow-up is running multiple seeds and reporting confidence intervals, since a single-seed result cannot support strong claims. A token-matched variant of Condition B would also help separate the effect of trajectory content from the effect of seeing more tokens. Beyond that, longer task streams and a semantic parameter scorer that distinguishes formatting errors from genuinely wrong tool calls would strengthen the evaluation. Replay or retrieval methods are worth exploring, though they would be new continual-learning interventions rather than part of the A/B supervision question.

\section{Contributions}

All three students contributed to the project. Individual responsibilities are listed in Table~\ref{tab:contrib}; shared work included project framing, result interpretation, and report writing.

\begin{table}[h]
\centering
\caption{Equal project contribution summary.}
\label{tab:contrib}
\small
...
\midrule
Vishnu Vardhan Reddy B & Data preprocessing pipeline, evaluation and scoring code, artifact validation, full generation evaluation runs. Shared: project framing, result interpretation, report writing. \\
Sagnik Chatterjee & Training notebook implementation, continual-learning metric computation, Colab experiment execution. Shared: project framing, result interpretation, report writing. \\
Soumik Bhatta & Error analysis, figure generation, presentation preparation, experiment verification. Shared: project framing, result interpretation, report writing. \\
\bottomrule
\end{tabular}
\end{table}
