...
Lightman et al.\ compare process and outcome supervision for mathematical reasoning and show that step-level supervision can be more effective than final-answer supervision \citep{lightman2023verify}. We borrow that distinction, but our data is different. API trajectories are not annotated reasoning steps; they are records of tool calls and tool observations. This makes the supervision weaker as a cognitive model but easier to collect in real systems.
...
Table~\ref{tab:contrib} lists individual responsibilities. Everyone shared project framing, result interpretation, and report writing.
...
\midrule
Vishnu Vardhan Reddy B & Data preprocessing pipeline, evaluation and scoring code, artifact validation, full generation evaluation runs. \\
Sagnik Chatterjee & Training notebook implementation, continual-learning metric computation, Colab experiment execution. \\
Soumik Bhatta & Error analysis, figure generation, presentation preparation, experiment verification. \\
\bottomrule
\end{tabular}
\end{table}
